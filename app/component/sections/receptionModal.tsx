...
export const ReceptionModal: React.FC<ReceptionModalProps> = ({
    ...
    const [imageDimensions, setImageDimensions] = useState<{ width: number; height: number }>({ width: 0, height: 0 });
    const [calculatedStyle, setCalculatedStyle] = useState<{ width: string; height: string }>({
        width: "0px",
        height: "0px",
    });
    ...
    useEffect(() => {
        const calculateStyle = () => {
            ...
            if (imageDimensions.width > maxWidth) {
                ...
                const height = (imageDimensions.height / imageDimensions.width) * width;
                setCalculatedStyle({ width: `${width}px`, height: `${height}px` });
            } else if (imageDimensions.height > maxHeight) {
                const height = maxHeight;
                const width = (imageDimensions.width / imageDimensions.height) * height;
                setCalculatedStyle({ width: `${width}px`, height: `${height}px` });
            } else {
                setCalculatedStyle({
